# RTI - Application Summary

> Consolidated analysis across all branches using 5-Tier AI Detection Model

**Generated:** 2026-01-16 11:40:24  
**Analysis Period:** 2025-12-01 to 2025-12-31

---

## Executive Summary

| Metric | Value |
|--------|-------|
| **Application** | RTI |
| **Branches Analyzed** | 1 |
| **Total Contributors** | 7 |
| **Overall AI Usage** | **45.52%** |

---

## Overall Statistics

### Code Attribution

| Category | Lines | Percentage | Commits |
|----------|-------|------------|---------|
| **AI-Assisted** | 5650 | 45.52% | 7 |
| **Human-Written** | 6762 | 54.48% | 108 |
| **Total** | 12412 | 100% | 115 |

### Visual Summary

```
AI-Assisted:   █████████░░░░░░░░░░░ 45.52%
Human-Written: ██████████░░░░░░░░░░ 54.48%
```

---

## 5-Tier Confidence Distribution

| Tier | Confidence | Description | Commits | Percentage |
|------|------------|-------------|---------|------------|
| **Tier 1** | 99-100% | Definitive AI markers | 0 | 0% |
| **Tier 2** | 90-98% | Very high confidence | 0 | 0% |
| **Tier 3** | 80-89% | High confidence | 3 | 42.9% |
| **Tier 4** | 70-79% | Moderate confidence | 1 | 14.3% |
| **Tier 5** | 60-69% | Low confidence | 3 | 42.9% |
| **Human** | <60% | No AI detected | 108 | 93.9% |

---

## Branch-Level Breakdown

| Branch | Commits | AI Commits | AI Lines | AI % | T1 | T2 | T3 | T4 | T5 | Contributors |
|--------|---------|------------|----------|------|----|----|----|----|-----|--------------|
| **main** | 115 | 7 | 5650 | 45.52% | 0 | 0 | 3 | 1 | 3 | 7 |

---

## Branch Reports

- **main**: [main/](main/) - AI: 45.52%

---

## Key Insights

### Human-Centric Development
This application is **primarily human-written** with only 45.52% AI-assisted code.
- Most commits are traditional human development
- AI tools used selectively or minimally

---

*Generated by Copilot Usage Report Tool - 5-Tier Analysis Model*
