# Container Migration - Application Summary

> Consolidated analysis across all branches using 5-Tier AI Detection Model

**Generated:** 2026-01-22 15:45:11  
**Analysis Period:** 2025-11-01 to 2025-11-30

---

## Executive Summary

| Metric | Value |
|--------|-------|
| **Application** | Container Migration |
| **Branches Analyzed** | 1 |
| **Total Contributors** | 12 |
| **Overall AI Usage** | **0.48%** |

---

## Overall Statistics

### Code Attribution

| Category | Lines | Percentage | Commits |
|----------|-------|------------|---------|
| **AI-Assisted** | 307 | 0.48% | 4 |
| **Human-Written** | 64084 | 99.52% | 35 |
| **Total** | 64391 | 100% | 39 |

### Visual Summary

```
AI-Assisted:   ░░░░░░░░░░░░░░░░░░░░ 0.48%
Human-Written: ███████████████████░ 99.52%
```

---

## 5-Tier Confidence Distribution

| Tier | Confidence | Description | Commits | Percentage |
|------|------------|-------------|---------|------------|
| **Tier 1** | 99-100% | Definitive AI markers | 3 | 75% |
| **Tier 2** | 90-98% | Very high confidence | 0 | 0% |
| **Tier 3** | 80-89% | High confidence | 0 | 0% |
| **Tier 4** | 70-79% | Moderate confidence | 1 | 25% |
| **Tier 5** | 60-69% | Low confidence | 0 | 0% |
| **Human** | <60% | No AI detected | 35 | 89.7% |

---

## Branch-Level Breakdown

| Branch | Commits | AI Commits | AI Lines | AI % | T1 | T2 | T3 | T4 | T5 | Contributors |
|--------|---------|------------|----------|------|----|----|----|----|-----|--------------|
| **dev** | 39 | 4 | 307 | 0.48% | 3 | 0 | 0 | 1 | 0 | 12 |

---

## Branch Reports

- **dev**: [dev/](dev/) - AI: 0.48%

---

## Key Insights

### Human-Centric Development
This application is **primarily human-written** with only 0.48% AI-assisted code.
- Most commits are traditional human development
- AI tools used selectively or minimally

---

*Generated by Copilot Usage Report Tool - 5-Tier Analysis Model*
