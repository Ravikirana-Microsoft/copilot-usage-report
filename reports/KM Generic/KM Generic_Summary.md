# KM Generic - Application Summary

> Consolidated analysis across all branches using 5-Tier AI Detection Model

**Generated:** 2026-04-21 05:39:21  
**Analysis Period:** 2025-10-01 to 2025-10-31

---

## Executive Summary

| Metric | Value |
|--------|-------|
| **Application** | KM Generic |
| **Branches Analyzed** | 1 |
| **Total Contributors** | 9 |
| **Overall AI Usage** | **5.61%** |

---

## Overall Statistics

### Code Attribution

| Category | Lines | Percentage | Commits |
|----------|-------|------------|---------|
| **AI-Assisted** | 120 | 5.61% | 1 |
| **Human-Written** | 2019 | 94.39% | 30 |
| **Total** | 2139 | 100% | 31 |

### Visual Summary

```
AI-Assisted:   █░░░░░░░░░░░░░░░░░░░ 5.61%
Human-Written: ██████████████████░░ 94.39%
```

---

## 5-Tier Confidence Distribution

| Tier | Confidence | Description | Commits | Percentage |
|------|------------|-------------|---------|------------|
| **Tier 1** | 99-100% | Definitive AI markers | 0 | 0% |
| **Tier 2** | 90-98% | Very high confidence | 0 | 0% |
| **Tier 3** | 80-89% | High confidence | 0 | 0% |
| **Tier 4** | 70-79% | Moderate confidence | 0 | 0% |
| **Tier 5** | 60-69% | Low confidence | 1 | 100% |
| **Human** | <60% | No AI detected | 30 | 96.8% |

---

## Branch-Level Breakdown

| Branch | Commits | AI Commits | AI Lines | AI % | T1 | T2 | T3 | T4 | T5 | Contributors |
|--------|---------|------------|----------|------|----|----|----|----|-----|--------------|
| **dev** | 31 | 1 | 120 | 5.61% | 0 | 0 | 0 | 0 | 1 | 9 |

---

## Branch Reports

- **dev**: [dev/](dev/) - AI: 5.61%

---

## Key Insights

### Human-Centric Development
This application is **primarily human-written** with only 5.61% AI-assisted code.
- Most commits are traditional human development
- AI tools used selectively or minimally

---

*Generated by Copilot Usage Report Tool - 5-Tier Analysis Model*
